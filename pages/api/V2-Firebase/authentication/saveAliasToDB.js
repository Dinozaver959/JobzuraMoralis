...
apiRoute.post(async (req, res) => {
  ...
  */
  ...
  if(isValid){
    ...
    await SaveAliasToDB(address.toLowerCase(), message.toLowerCase());
    console.log("Alias saved to DB");
...
async function SaveAliasToDB(address, alias){
  ...
  await admin.firestore().collection('aliases').doc(alias).set(AliasRecord, { merge: true });
}