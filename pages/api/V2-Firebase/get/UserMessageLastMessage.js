...
apiRoute.get(async (req, res) => {     
  ...
  const jobs = await GetUserMessageLastMessage(UserWallet.toLowerCase());
...
async function GetUserMessageLastMessage(userWallet){
  const res = await admin.firestore().collection('messages').doc(userWallet).collection(userWallet).get();
