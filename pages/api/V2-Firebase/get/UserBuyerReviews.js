...
apiRoute.get(async (req, res) => {     
  ...
  const jobs = await GetUserBuyerReviews(UserWallet.toLowerCase());
...
async function GetUserBuyerReviews(userWallet) {
  const res = await admin.firestore().collection(`reviewsToBuyers`).where('BuyerWallet', '==', userWallet).get();
